...
/**
 * Lấy đối tượng PDO kết nối đến cơ sở dữ liệu
 * 
...
/**
 * Thực thi một truy vấn và trả về một kết quả duy nhất
 * 
 * @param string $query Câu truy vấn SQL
 * @param array $params Tham số của truy vấn (nếu có)
 * @return array|bool Kết quả duy nhất hoặc false nếu không có
 */
...
/**
 * Thực thi một truy vấn không trả về kết quả (INSERT, UPDATE, DELETE)
 * 
 * @param string $query Câu truy vấn SQL
 * @param array $params Tham số của truy vấn (nếu có)
...
function db_execute($query, $params = []) {
    try {
        ...
        error_log("Lỗi thực thi: " . $e->getMessage() . " - Query: $query");
        if (defined('DEBUG_MODE') && DEBUG_MODE) {
...
/**
 * Chèn dữ liệu vào bảng và trả về ID được tạo (nếu có)
 * 
